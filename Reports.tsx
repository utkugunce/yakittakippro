...
export const Reports: React.FC<ReportsProps> = ({ logs, maintenanceItems = [], vehicleParts = [] }) => {
  const generateSalesReport = () => {
    ...
    doc.text("Araç Satış Raporu", 14, 20);
    ...
    const avgCons = logs.length > 0 ? (logs.reduce((acc, l) => acc + l.dailyFuelConsumed, 0) / totalDist) * 100 : 0;
    ...
    autoTable(doc, {
      ...
      body: [
        ...
        ['Toplam Kayıt Sayısı', `${logs.length} adet`]
      ],
      theme: 'striped',
    ...
    doc.setFontSize(14);
    doc.setTextColor(40, 40, 40);
    doc.text("Bakım Geçmişi", 14, lastTableY);

    const maintRows = maintenanceItems.map(m => [
      m.title,
      m.lastMaintenanceKm ? `${m.lastMaintenanceKm} km` : '-',
      m.nextDueKm ? `${m.nextDueKm} km` : (m.dueDate || '-'),
      m.status === 'ok' ? 'Yapıldı/Normal' : 'Gecikmiş/Yaklaşıyor'
    ]);

    autoTable(doc, {
      startY: lastTableY + 5,
      head: [['Bakım Adı', 'Son Yapılan', 'Bir Sonraki', 'Durum']],
      body: maintRows.length ? maintRows : [['Kayıt Bulunamadı', '-', '-', '-']],
      theme: 'grid',
      headStyles: { fillColor: [46, 204, 113] }
    });

    // Part History
    const partsY = (doc as any).lastAutoTable.finalY + 15;
    doc.setFontSize(14);
    doc.text("Parça & Lastik Durumu", 14, partsY);

    const partRows = vehicleParts.map(p => [
      p.type.toUpperCase(),
      p.name,
      `${p.installKm} km`,
      p.lifespanKm ? `%${Math.min(100, ((lastOdometer - p.installKm) / p.lifespanKm) * 100).toFixed(0)} Kullanıldı` : 'Ömür Belirsiz',
      p.isActive ? 'Aktif' : 'Depoda'
    ]);

    // Need lastOdometer for calc above, finding it from max logs
    const lastOdometer = logs.length > 0 ? Math.max(...logs.map(l => l.currentOdometer)) : 0;

    // Recalculate usage with found odometer
    const partRowsCorrected = vehicleParts.map(p => [
      p.type.toUpperCase(),
      p.name,
      `${p.installKm} km`,
      p.lifespanKm ? `%${Math.min(100, ((lastOdometer - p.installKm) / p.lifespanKm) * 100).toFixed(0)} Kullanıldı` : '-',
      p.isActive ? 'Aktif' : 'Depoda'
    ]);

    autoTable(doc, {
      startY: partsY + 5,
      head: [['Tip', 'Parça Adı', 'Takılma KM', 'Durum', 'Konum']],
      body: partRowsCorrected.length ? partRowsCorrected : [['Kayıt Bulunamadı', '-', '-', '-', '-']],
      theme: 'grid',
