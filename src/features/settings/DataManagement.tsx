...
export const DataManagement: React.FC<DataManagementProps> = ({ logs, onImport, onClear }) => {
    const { repairFuelPrices } = useAppStore();
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Repair Fuel Prices */}
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-amber-100 dark:border-amber-900/30 overflow-hidden">
                <div className="p-4 border-b border-amber-100 dark:border-amber-900/30 bg-amber-50/50 dark:bg-amber-900/10 flex items-center gap-2">
                    <div className="p-2 bg-amber-100 dark:bg-amber-900/30 rounded-lg">
                        <Wrench className="w-5 h-5 text-amber-600 dark:text-amber-400" />
                    </div>
                    <h3 className="font-bold text-amber-900 dark:text-amber-200">Yakıt Fiyatı Onarımı</h3>
                </div>

                <div className="p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
                    <p className="text-sm text-gray-600 dark:text-gray-300">
                        Günlük kayıtlardaki yakıt fiyatlarını, yakıt alım geçmişinize göre otomatik olarak eşleştirir ve düzeltir.
                    </p>
                    <button
                        onClick={() => {
                            if (window.confirm('Yakıt fiyatları, yakıt alım geçmişinize göre otomatik eşleştirilecek. Devam etmek istiyor musunuz?')) {
                                const count = repairFuelPrices();
                                setImportStatus({
                                    success: true,
                                    message: count > 0 ? `${count} kaydın yakıt fiyatı başarıyla güncellendi.` : 'Tüm kayıtlar zaten doğru fiyatlarla eşleşiyor.'
                                });
                            }
                        }}
                        className="w-full sm:w-auto px-5 py-2.5 bg-amber-500 hover:bg-amber-600 text-white font-bold rounded-xl shadow-lg shadow-amber-500/20 transition-all hover:-translate-y-0.5 flex items-center justify-center gap-2"
                    >
